...
## Performance
 ...
 - `expandTab` and `unexpandTab` functions call `editorRowInsertChar` and `editorTowDelChar`, which reallocates the entire row's character buffer, meaning that files with lots of tabs causes performance to suffer. (Partially mitigated: csize field provides 50% excess capacity allocation to reduce realloc frequency)
